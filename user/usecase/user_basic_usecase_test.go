...
func TestUserUsecase_Store(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert := assert.New(t)
			umsqlr := &_mocks.UserMysqlRepository{}
			urdsr := &_mocks.UserRedisRepository{}
			uu := NewUserUsecase(umsqlr, urdsr)
			if !tt.wantErr {
				umsqlr.On("Store", context.Background(), tt.args.user).Return(tt.wants.user, nil)
			} else {
				umsqlr.On("Store", context.Background(), tt.args.user).Return(tt.wants.user, errors.New("failed to store user"))
			}
